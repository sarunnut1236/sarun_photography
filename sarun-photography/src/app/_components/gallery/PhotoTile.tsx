...
export default function PhotoTile({ src, alt, description }: PhotoTileProps) {
  ...
  return (
    <div className="group relative aspect-square overflow-hidden rounded-xl border border-transparent bg-(--bg) transition-transform duration-300 group-hover:scale-[1.02]">
      {shouldShow && (
        <CldImage
          src={src}
          alt={alt}
      ...
      <div className="pointer-events-none absolute inset-0 flex items-end bg-linear-to-t from-black/60 via-black/20 to-transparent opacity-0 transition-opacity group-hover:opacity-100">
        <div className="pointer-events-auto w-full px-3 pb-3 text-xs text-white">
          {description}
        </div>
      </div>
    </div>
